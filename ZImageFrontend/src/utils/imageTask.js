...
export const isImageActiveStatus = (status) =>
  IMAGE_ACTIVE_STATUSES.includes(normalizeImageStatus(status));

export const isImageTerminalStatus = (status) =>
  IMAGE_TERMINAL_STATUSES.includes(normalizeImageStatus(status));

export const canCancelImageTask = (status) =>
  IMAGE_ACTIVE_STATUSES.includes(normalizeImageStatus(status));
...
export const getImageStatusTagType = (status) => {
  ...
  if (normalized === 'queued' || normalized === 'pending' || normalized === 'generating')
    return 'warning';
