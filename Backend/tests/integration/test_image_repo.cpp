...
template <typename T> T repoValue(RepoResult<T> result) {
    if (!result) {
        ADD_FAILURE() << "Repo error: " << result.error().message;
        return T{};
    }
...
TEST_F(ImageRepoTest, FindByIdWrongUserReturnsNullopt) {
    ...
    auto fetched = repoValue(repo_.findByIdAndUserId(id, kTestUserId + 1));
    EXPECT_FALSE(fetched.has_value());
}

TEST_F(ImageRepoTest, FindByIdNonexistentReturnsNullopt) {
    auto fetched = repoValue(repo_.findByIdAndUserId(999999, kTestUserId));
    EXPECT_FALSE(fetched.has_value());
}
...
TEST_F(ImageRepoTest, FindByUserIdPaginated) {
    for (int i = 0; i < 5; ++i) {
        ...
        repoValue(repo_.insert(gen));
    }

    auto page0 = repoValue(repo_.findByUserId(kTestUserId, 0, 3));
    ...
    auto page1 = repoValue(repo_.findByUserId(kTestUserId, 1, 3));
    EXPECT_EQ(page1.content.size(), 2u);
    EXPECT_EQ(page1.total_elements, 5);
}

TEST_F(ImageRepoTest, FindByUserIdEmptyResult) {
    auto page = repoValue(repo_.findByUserId(kTestUserId, 0, 10));
...
TEST_F(ImageRepoTest, FindByUserIdDifferentUsersIsolated) {
    ...
    repoValue(repo_.insert(gen1));
    auto gen2 = makeGen(kTestUserId + 1, "user 43 task");
    repoValue(repo_.insert(gen2));

    auto page42 = repoValue(repo_.findByUserId(kTestUserId, 0, 10));
    EXPECT_EQ(page42.total_elements, 1);
    EXPECT_EQ(page42.content[0].prompt, "user 42 task");

    auto page43 = repoValue(repo_.findByUserId(kTestUserId + 1, 0, 10));
...
TEST_F(ImageRepoTest, FindByUserIdAndStatusFilter) {
    repoValue(repo_.insert(makeGen(kTestUserId, "q1", models::TaskStatus::Queued)));
    repoValue(repo_.insert(makeGen(kTestUserId, "q2", models::TaskStatus::Queued)));
    repoValue(repo_.insert(makeGen(kTestUserId, "ok", models::TaskStatus::Success)));
    repoValue(repo_.insert(makeGen(kTestUserId, "fail", models::TaskStatus::Failed)));

    auto queued =
        repoValue(repo_.findByUserIdAndStatus(kTestUserId, models::TaskStatus::Queued, 0, 10));
    ...
    auto success =
        repoValue(repo_.findByUserIdAndStatus(kTestUserId, models::TaskStatus::Success, 0, 10));
    EXPECT_EQ(success.total_elements, 1);
    EXPECT_EQ(success.content[0].prompt, "ok");

    auto cancelled =
        repoValue(repo_.findByUserIdAndStatus(kTestUserId, models::TaskStatus::Cancelled, 0, 10));
...
TEST_F(ImageRepoTest, FindByRequestIdAndUserIdMatch) {
    ...
    auto found = repoValue(repo_.findByRequestIdAndUserId("dedup-abc", kTestUserId));
...
TEST_F(ImageRepoTest, FindByRequestIdWrongUserReturnsNullopt) {
    ...
    auto found = repoValue(repo_.findByRequestIdAndUserId("dedup-xyz", kTestUserId + 1));
    EXPECT_FALSE(found.has_value());
}

TEST_F(ImageRepoTest, FindByRequestIdNotFoundReturnsNullopt) {
    auto found = repoValue(repo_.findByRequestIdAndUserId("nonexistent", kTestUserId));
    EXPECT_FALSE(found.has_value());
}
...
TEST_F(ImageRepoTest, ClaimNextTaskAtomically) {
    repoValue(repo_.insert(makeGen(kTestUserId, "first")));
    repoValue(repo_.insert(makeGen(kTestUserId, "second")));
    repoValue(repo_.insert(makeGen(kTestUserId, "third")));

    auto claimed1 = repoValue(repo_.claimNextTask("worker1", 60));
    ...
    // Claim second task
    auto claimed2 = repoValue(repo_.claimNextTask("worker2", 60));
...
TEST_F(ImageRepoTest, ClaimNextTaskEmptyQueue) {
    auto result = repoValue(repo_.claimNextTask("worker1", 60));
...
TEST_F(ImageRepoTest, ClaimTaskByIdSuccess) {
    ...
    auto claimed = repoValue(repo_.claimTaskById(id, "worker-xyz", 120));
...
TEST_F(ImageRepoTest, ClaimTaskByIdAlreadyClaimedFails) {
    ...
    ASSERT_TRUE(repoValue(repo_.claimTaskById(id, "worker-a", 60)).has_value());
    // Second claim should fail — status is now 'generating', not 'queued'/'pending'
    auto secondClaim = repoValue(repo_.claimTaskById(id, "worker-b", 60));
    EXPECT_FALSE(secondClaim.has_value());
}

TEST_F(ImageRepoTest, ClaimTaskByIdNonexistentFails) {
    auto claimed = repoValue(repo_.claimTaskById(999999, "worker-x", 60));
...
TEST_F(ImageRepoTest, CancelByIdAndUserIdQueued) {
    ...
    auto updated = repoValue(repo_.cancelByIdAndUserId(id, kTestUserId));
...
TEST_F(ImageRepoTest, CancelByIdAndUserIdWrongUserFails) {
    ...
    auto result = repoValue(repo_.cancelByIdAndUserId(id, kTestUserId + 1));
    EXPECT_FALSE(result.has_value());
}
...
TEST_F(ImageRepoTest, CancelByIdAndUserIdAlreadyCancelledFails) {
    ...
    ASSERT_TRUE(repoValue(repo_.cancelByIdAndUserId(id, kTestUserId)).has_value());
    // Second cancel — status is now 'cancelled', not in (queued, pending, generating)
    auto result = repoValue(repo_.cancelByIdAndUserId(id, kTestUserId));
    EXPECT_FALSE(result.has_value());
}
...
TEST_F(ImageRepoTest, RetryByIdAndUserIdFailed) {
    ...
    auto updated = repoValue(repo_.retryByIdAndUserId(id, kTestUserId));
    ASSERT_TRUE(updated.has_value());
...
TEST_F(ImageRepoTest, RetryByIdAndUserIdQueuedFails) {
    ...
    auto result = repoValue(repo_.retryByIdAndUserId(id, kTestUserId));
    EXPECT_FALSE(result.has_value());
}
...
TEST_F(ImageRepoTest, DeleteByIdAndUserIdTerminalStatus) {
    ...
    bool result = repoValue(repo_.deleteByIdAndUserId(id, kTestUserId));
    EXPECT_TRUE(result);
...
TEST_F(ImageRepoTest, FinishClaimedTaskLifecycle) {
    ...
    int64_t id = repoValue(repo_.insert(gen));
    auto claimed = repoValue(repo_.claimTaskById(id, "worker-lifecycle", 300));
    ASSERT_TRUE(claimed.has_value());
...
TEST_F(ImageRepoTest, FinishClaimedTaskWrongWorkerFails) {
    ...
    int64_t id = repoValue(repo_.insert(gen));
    auto claimed = repoValue(repo_.claimTaskById(id, "real-worker", 300));
    ASSERT_TRUE(claimed.has_value());
...
TEST_F(ImageRepoTest, RenewLease) {
    ...
    int64_t id = repoValue(repo_.insert(gen));
    auto claimed = repoValue(repo_.claimTaskById(id, "worker-lease", 60));
    ASSERT_TRUE(claimed.has_value());

    bool renewed = repoValue(repo_.renewLease(id, kTestUserId, "worker-lease", 300));
    EXPECT_TRUE(renewed);
}
...
TEST_F(ImageRepoTest, RenewLeaseWrongWorkerFails) {
    ...
    bool renewed = repoValue(repo_.renewLease(id, kTestUserId, "worker-other", 300));
...
TEST_F(ImageRepoTest, RenewLeaseNonGeneratingFails) {
    ...
    bool renewed = repoValue(repo_.renewLease(id, kTestUserId, "worker-x", 60));
    EXPECT_FALSE(renewed);
}
...
TEST_F(ImageRepoTest, FindQueuedTaskIds) {
    repoValue(repo_.insert(makeGen(kTestUserId, "q1", models::TaskStatus::Queued)));
    repoValue(repo_.insert(makeGen(kTestUserId, "q2", models::TaskStatus::Pending)));
    repoValue(repo_.insert(makeGen(kTestUserId, "ok", models::TaskStatus::Success)));
    repoValue(repo_.insert(makeGen(kTestUserId, "fail", models::TaskStatus::Failed)));
    repoValue(repo_.insert(makeGen(kTestUserId, "gen", models::TaskStatus::Generating)));
    repoValue(repo_.insert(makeGen(kTestUserId, "cancelled", models::TaskStatus::Cancelled)));

    auto ids = repoValue(repo_.findQueuedTaskIds());
